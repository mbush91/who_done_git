...
def print_summary(committers: Dict[str, Counter]):
    ...
    users_sorted = sorted(user_files.items(), key=lambda item: -sum(f[2] for f in item[1]))
...
def main():
    parser = argparse.ArgumentParser(description="Summarize git committers for files in a directory.")
    ...
    try:
        ...
        tracked_files = set(os.path.normpath(os.path.join(git_root, f)) for f in tracked.stdout.strip().split('\n') if f)
